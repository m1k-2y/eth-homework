...
-->B. EVM에서 랜덤함수나 외부API 호출을 허용할 시 노드들간에 서로 다른 결과를 얻을 수 있어 상태가 다를 수 있다. 
그렇기에 EVM이 결정론적이여야 하는 이유는 모든 노드가 같은 입력에 같은 결과를 얻어 합의를 해야하기 때문이다.
...
-->B. Storage는 온체인에 저장되는 데이터로 영구저장이므로 가장 비싸다. uint[] memory data를 보았을때 data는 memory로
memory는 함수 실행시 임시적으로 저장되는 것이다. Stack은 내부에서 계산과 같은 간단한 연산 처리용으로 가장 빠르고 
싸다. calldata는 트랜잭션이 들고온 원본 데이터로 이 또한 싸며 원본이기에 수정이 불가능하다.
...
-->D. Storage를 읽고 쓰는 것은 단순한 연산이 아닌 전 세계에 노드의 상태를 변경하고 조회하는 것이다. 그렇기에 메모리에서
일어나는 덧셈,곱셈은 저렴하지만 storage 읽기, 쓰기는 상대적으로 더 비싸다. 특히 쓰기는  모든 노드의 상태를 변경해야 
하기에 가장 비용이 높다
...
-->재진입 공격은 외부 호출 후 상태 변경이 되기 전에 recieve함수를 통해 계속해서 withdraw함수를 실행시킨다. 그렇지만
CEI에서 상태변경이 먼저 온다면 자신이 deposit한 돈을 초과해서 recieve 할 수 없다.
...
- 기술적 교훈 (코드 작성 관점)
- 생태계 교훈 (이더리움 커뮤니티 관점)
-->기술적 교훈으로는 어느 코드든 논리적 취약점이 존재할 수 있음을 인지하고 있어야 한다라고 생각한다. 이 사건 이후
이더리움 생태계는 이더리움 클래식과 이더리움으로 나뉘었다. 이더리움 클래식은 코드는 법이다와 같은 불변성과 절대주의를
지지하며 하드포크 된 이더리움은 블록체인을 사회적 시스템으로 바라보아 합의를 통한 상태 변경이 가능하다고 본 것 같다.
...
-->withdraw함수에서 외부 호출 후 상태 변경이 일어나고 있다.


**2) 왜 이것이 문제인가:**
<!--
공격자가 어떻게 이 취약점을 악용할 수 있는지 단계별로 설명하세요.
-->외부 호출 후 상태변경이 일어나기 전이기에 require(balances[msg.sender] >= amount, 가 아직 성립하여 재진입 공격을 
통해 계속해서 출금을 진행할 수 있다.
...
function secureWithdraw(uint256 amount) public {
    // 1. Checks - 조건 확인
    require(balnces[msg.sender] >= amount, "Insufficient balance");
...
-->외부 호출 이전에 상태변경을 먼저 함으로서 외부 호출을 한 후 또다시 재진입을 하더라도 조건 확인에 성립되지 않아
실패하기 때문이다.
...
-->tx.origin은 처음으로 코드를 실행시킨 주소이고 msg.sender는 실행중인 코드를 직전에 실행시킨 주소이다. 그래서 피해자가
처음 위의 코드를 실행시키기만 하면 중간에 공격자가 끼어들어 악성 코드를 실행시키더라도 tx.origin == owner가 성립되기에
문제가 된다.
...
힌트: 공격자 컨트랙트를 통한 우회
-->위에서 설명하였듯이 피해자를 속여 코드를 처음으로 실행시키게 한 후 중간에 공격자가 끼어들어 transferOwnership을 통해
owner를 변경시킬 수도 있다.
...
<!--
두 방법의 장단점을 설명하세요.
-->CEI 패턴은 단순히 순서를 바꾸었기에 추가 연산이 없어 가스가 효율적이며 외부 라이브러리를 불러올 필요가 없다. 그러나
순서를 바꿔야 한다는 점에서 개발자가 순서를 직접 관리해야하며 실수가 있을 수 있다. ReentrancyGuard는 검증된 라이브러리
modifier를 사용하기에 안전하고 실수를 방지할 수 있다. 그러나 가스를 더 써야하며 외부에 의존해야 한다는 단점이 있다.
...
-->상태변경이 외부 호출이 발생하고 나서야 일어나기에 중간에 재진입을 하더라도 잔액이 아직 1 ETH로 기록되어 있어
require 체크가 통과한다.
...
-->이미 상태 변경이 되고 외부호출을 한 후에 recieve를 트리거하면 require를 통과할 수 없기에 재진입을 할 수 없다.
...
-->VulnerableVault가 가진 10 ETH가 전부 바닥날때까지 탈취 가능하므로 총 10 ETH를 탈취할 수 있다.
